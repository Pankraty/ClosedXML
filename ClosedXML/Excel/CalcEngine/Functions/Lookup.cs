...
namespace ClosedXML.Excel.CalcEngine.Functions
{
    internal static class Lookup
    {
        ...
        private static object Hlookup(List<Expression> p)
        {
            ...
            matching_column = range.FindColumn(c => !c.Cell(1).IsEmpty() && new Expression(c.Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) == 0);
            if (range_lookup && matching_column == null)
            {
                ...
                matching_column = range.FindColumn(c =>
                {
                    ...
                    if (column_index_in_range < number_of_columns_in_range && !c.Cell(1).IsEmpty() && new Expression(c.Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) <= 0 && !c.ColumnRight().Cell(1).IsEmpty() && new Expression(c.ColumnRight().Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) > 0)
                        return true;
                    else if (column_index_in_range == number_of_columns_in_range && !c.Cell(1).IsEmpty() && new Expression(c.Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) <= 0)
                        return true;
                    else
                        return false;
                });
        ...
        private static object Match(List<Expression> p)
        {
            ...
            if (match_type == 0)
                foundCell = range
                    .CellsUsed(XLCellsUsedOptions.Contents, c => lookupPredicate.Invoke(new Expression(c.Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value)))
                    .FirstOrDefault();
            else
            {
                ...
                foundCell = range
                    ...
                    .TakeWhile(c =>
                    {
                        var currentCellExpression = new Expression(c.Value, CoersionConvention.CaseInsensitive);

                        if (previousValue != null)
                        {
                            // When match_type != 0, we have to assume that the order of the items being search is ascending or descending
                            var previousValueExpression = new Expression(previousValue, CoersionConvention.CaseInsensitive);
        ...
        private static object Vlookup(List<Expression> p)
        {
            ...
            try
            {
                matching_row = range.FindRow(r => !r.Cell(1).IsEmpty() && new Expression(r.Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) == 0);
            }
            catch (Exception ex)
            {
            ...
            if (range_lookup && matching_row == null)
            {
                ...
                matching_row = range.FindRow(r =>
                {
                    ...
                    if (row_index_in_range < number_of_rows_in_range && !r.Cell(1).IsEmpty() && new Expression(r.Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) <= 0 && !r.RowBelow().Cell(1).IsEmpty() && new Expression(r.RowBelow().Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) > 0)
                        return true;
                    else if (row_index_in_range == number_of_rows_in_range && !r.Cell(1).IsEmpty() && new Expression(r.Cell(1).Value, CoersionConvention.CaseInsensitive).CompareTo(lookup_value) <= 0)
                        return true;
                    else
                        return false;
                });
